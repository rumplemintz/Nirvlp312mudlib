...
set_player_attributes( object walker ) {
  ...
  switch( ex_level ) {
    ...
    case 51..60:
      ...
      break;
    case 61..75:
      walker->set_walker_level( 27 );
      strs += "Huge";
      break;
    case 76..90:
      walker->set_walker_level( 28 );
      strs += "Scary";
      break;
    case 91..99:
      walker->set_walker_level( 29 );
      strs += "Fearsome";
      break;
    case 100..1000:
      walker->set_walker_level( 30 );
      strs += "Terrifying";
      break;
  }
   
  walker->set_player_flag();
  walker->set_name( player_name );
  walker->set_gender( gender );
  walker->add_walker_id( player_name );
  strs += " Walking Dead "+NORM+capitalize(player_name) + " " + title;
  if( pretitle )
    strs = pretitle + " " + strs;
  if( al_title )
    strs += " ("+al_title+")";
