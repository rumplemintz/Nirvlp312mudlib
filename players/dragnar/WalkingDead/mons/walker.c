 ...
 *
 * Purpose: The main walker mob.  Can be between level 20 and 26 depending on
 *          the number of walkers that have joined into the mob.  Has higher
 *          than average WC/AC and very high heal rate.  Strong spells, with
 *          spells randomly attacking pets/players etc.  "groan" spell calls
 *          nearby walkers, which are aggro, which is very deadly.  
 * History:
 *          06/09/2013 - Gnar
 *            Created
 *          10/16/2015 - Gnar
 *            Added exp to the custom corpse in monster_died.
 ******************************************************************************/
static status registered = 0;

static int walker_level, player_flag;

int height_ft, height_in, weight;
...
query_player_flag() { return player_flag; }
set_player_flag() { player_flag = 1; }
