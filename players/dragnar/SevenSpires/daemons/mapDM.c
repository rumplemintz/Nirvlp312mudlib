...
void load_map( string str ) {
  int i;
  string tmp_dir, upper_dir,file_name, color;
  string *file_path;
  ...
  file_path = explode(tmp_dir, "/" );
  file_path -= ({ file_path[ sizeof(file_path) - 1 ] });
  upper_dir = implode( file_path , "/" ) + "/";
#if DEBUG
  write("Dir:   "+tmp_dir+".\n");
  write("Upper: "+upper_dir+"\n");
  write("FN:    "+file_name+".\n");
#endif
...
#if DEBUG
  write( "Key: " +  upper_dir + "map_key.\n");
#endif
  if( !restore_object( upper_dir + "map_key") )
    return;
  
#ifndef __LDMUD__
  map_key = reconstruct_mapping( map_key_a );
  /****************************************************************************
   *  Bug with LP, unable to store "\" in a .o file, so we store BS for back-
   * slash in the .o file, and replace it here. Works ok on LD
   ****************************************************************************/
#if 1
  if( member( map_key, "BS" ) ) {
    map_key += ([ "\^" : map_key[ "\^" ] ]);
  }
#endif
#endif
