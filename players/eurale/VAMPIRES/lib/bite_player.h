...
if(target_obj->query_level() < 4) {
  ...
  if (target_obj->query_level() > 19 &&
      (this_player()->query_real_name() != "khrell" ||
      this_player()->query_real_name() != "fred" ||
  ...
  if (present("vampire fangs", this_player()) &&
      ...
      (this_player()->query_real_name() != "khrell" &&
       this_player()->query_real_name() != "oberon" &&
...
/*  Beer  5/24/03   */
if(TPRN == "beer") {
  write("You can no longer bite, contact Khrell if you have questions.\n");
  return 1; }
