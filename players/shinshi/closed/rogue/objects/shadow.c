...
short(){
	return weapon->short()+" "+HIG+"\[coated in deadly poison\]"+NORM;
}
...
weapon_hit(atk)
{
	...
	if(number > 85 && !present("poison_damage_yo", atk)){
		poison = clone_object(OBJPATH+"poison.c");
		move_object(poison, atk);
		poison->start_it();
		/*tell_object(find_player("shinshi"), "DEBUG: DEADLY Poison put on the mob.\n");	For debugging purposes */
	}
