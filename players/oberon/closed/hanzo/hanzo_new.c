...
reset(arg) {
	...
	SetMultipleIds(({"hanzo_sword"}));
	set_alias("hanzo sword");
	set_type("sword");
	set_short("A Hanzo sword");
	set_long("A 27\" long blade hewn from the finest stainless Japanese steel,\n"+
...
query_message_hit(tmp) {
	...
	if (tmp < 40) { mess = ({"'s "+bp,message_hit[2]}); }
	if (tmp < 35) { mess = ({"'s "+bp,HIK+"sliced"+NORM,}); }
	if (tmp < 30) { mess = ({"'s "+bp,RED+"cut"+NORM,}); }
	if (tmp < 25) { mess = ({"'s "+bp,message_hit[4]}); }
	if (tmp < 20) { mess = ({"'s "+bp,YEL+"filleted"+NORM }); }
	if (tmp < 15) { mess = ({"'s "+bp,message_hit[6]}); }
	if (tmp < 10) { mess = ({"'s "+bp,message_hit[8]}); }
	if (tmp < 7) { mess = ({"'s "+bp,"delicately traced a jagged line across"}); }
	if (tmp < 4) { mess = ({"'s "+bp,message_hit[10]}); }
	if (tmp == 1) { mess = ({"'s "+bp,message_hit[12]}); }
	return mess;
}
...
get_style() {
	string typ;
	switch(random(14)) {
		case 0: typ = "Muso Jikiden Eishin Ryu"; break;
		case 1: typ = "Omori Ryu"; break;
		case 2: typ = "Iaido"; break;
		case 3: typ = "Iaijitsu"; break;
		case 4: typ = "Tenshin Shoden Katori Shinto Ryu"; break;
		case 5: typ = "Muso Ryu of Hayashizaki Jinsuke Shigenobu"; break;
		case 6: typ = "Shinkendo"; break;
		case 7: typ = "Tameshi-giri"; break;
		case 8: typ = "Dai Nihon Batto Ho"; break;
		case 9: typ = "Nakanishi-ha Itto Ryu"; break;
		case 10: typ = "Abe Ryu"; break;
		case 11: typ = "Yagyu Shinkage"; break;
		case 12: typ = "Niten Ichi"; break;
		case 13: typ = "Yagyu Shingan"; break;
	}
	return typ;
}
...
deathblow(atk) {
...

	{
		...
		obj->set_long("A bloody, decapitated head.\n");
	...
	if(wielded_by->query_attrib("dex") > random(50)) {
		...
		while(times-- && atk) {
			...
			tell_room(environment(atk),
				...
				BOLD+typ+NORM+".\n",
				({ wielded_by }));
			tell_object(wielded_by, "You invoke the Style of "+BOLD+typ+NORM+".\n");
			...
			atk->hit_player(1 + (spec * 3 / 4));
		}
		if(atk) {
			object wep;
			if((wep = atk->query_weapon()) || (atk->is_npc() && (wep=present("weapon", atk)))) {
