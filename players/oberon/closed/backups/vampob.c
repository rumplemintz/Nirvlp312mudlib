...
extra_look(string str) {
  if(this_player()==environment()) {
    write("~o~ You are a Vampire! ~o~\n");
  }
  else if(this_player() && present("vampire fangs", this_player())) {
    write("~o~ "+(string)environment()->query_name()+" is a fellow Vampire! ~o~\n");
  }
}
...
set_telepathy(arg) { 
  telepathy_on = arg; 
  if(arg && !CHAN->query_channel(CNAME, environment())) CHAN->add_channel(CNAME, environment());
  if(!arg && CHAN->query_channel(CNAME, environment())) CHAN->remove_channel(CNAME, environment());
}
...
long() {
  ...
  "from out of your upper gums.\n\nThe guild help command is: "+HIR+"vhelp\n"+NORM);
}
...
/*
        call_other(VMEMBER, "???", 0);
        vampire = call_other(VMEMBER, "query_member", TPRN);
        if(!vampire && (int)this_player()->query_level() < 1000) { destruct(this_object()); return 1; }
*/
        ...
        /* updatedvampire strength code inserted by illarion 2 dec 2004 */
        TP->RegisterWeaponBonus(this_object(),0,"vampire_strength_bonus");
        set_telepathy(telepathy_on);
...
attack_backhand(str) { return BASE->attack_backhand(str); }
attack_damn(str) { return BASE->attack_damn(str); }
attack_demon(str) { return BASE->attack_demon(str); }
attack_grasp(str) { return BASE->attack_grasp(str); }
attack_scratch(str) { return BASE->attack_scratch(str); }
...
/* set_wc has no effect on players at this time
  I am commenting this out and replacing it with
  RegisterWeaponBonus code.
                illarion 2 dec 2004
...
/* anti div by 0 check added by illarion 5 dec 2004 */
if(!vdiv || (damage + 10)/vdiv > target) return;
  if(num < speed) {
...
vampire_strength_bonus() {
  int adjwc;
  if(environment()->query_level() > 19)
    tell_object(environment(),"Vampire strength bonus.\n");
  if(environment()->query_guild_exp() == 666) adjwc = 0;
  else if(environment()->query_guild_exp() == 667 ||
          environment()->query_guild_exp() == 668) adjwc = 1;
  else if(environment()->query_guild_exp() == 669) adjwc = 2;
  else if (environment()->query_guild_exp() >= 670) adjwc = 6;
/* Changed from >=369 to >=670 - typo? - Rumplemintz */
  return (adjwc*3); /* doubled by vertebraker */
  /* then tripled */
}
monitor_code(str) {
  object att;
  att=environment()->query_attack();
  switch(str) {
    case "damn":
      if(att && present("vamp_damn_obj",att))
        return HIW+"["+HIB+"Damned"+HIW+"]"+NORM;
      else return "";
    break;
  }
}
...
query_gmon1() {
	if(TPBPTS > 1250) return "GODLIKE";
	else if(TPBPTS > 1000 && TPBPTS <= 1250) return "UNBELIEVABLE";
	else if(TPBPTS > 750 && TPBPTS <= 1000) return "EXTREME";
	else if(TPBPTS > 550 && TPBPTS <= 750) return "HUGE";
	else if(TPBPTS > 450 && TPBPTS <= 550) return "ROBUST";
	else if(TPBPTS > 350 && TPBPTS <= 450) return "HIGH";
	else if(TPBPTS > 250 && TPBPTS <= 350) return "MEDIUM";
	else if(TPBPTS > 150 && TPBPTS <= 250) return "LOW";
	else if(TPBPTS > 50 && TPBPTS <= 150) return "ANEMIC";
	else if(TPBPTS <= 50) return "CRITICAL";
}
