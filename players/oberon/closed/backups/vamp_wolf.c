/* Edited by Vital                    */
/*   added room boost for wolf rock   */
/* Edited by Oberon                   */
/*   boosted wolf lvl to 10          */
...
follow(str)
{
  ...
  if(master)
    ...
    {
      tell_room(environment(TO), "The " + BOLD + "wolf" + NORM +
                " lopes away after " + capitalize(master_name) + ".\n");
      move_object(TO, environment(master));
      tell_room(environment(master), "A " + BOLD + "wolf" + NORM +
...
reset(arg) {
	...
	set_ac(10);
  if(!present("wolfangs")) {
    move_object(clone_object("/players/eurale/VAMPIRES/OBJ/wolf_fangs.c"), TO);
  }
