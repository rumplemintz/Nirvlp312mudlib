...
cmd_list() {
	...
	write("|                                           |\n");
	write("|  Sword of Healing      (heal)     $"+COST+"   |\n");
	write("|  Sword of Detoxing     (detox)    $"+COST+"   |\n");
	write("+-------------------------------------------+\n");
	return 1;
}
...
cmd_buy(arg) {
	...
	switch(arg) {
		case "heal":
			giveitem = clone_object("/players/oberon/amber/city/obj/sword_heal.c");
			...
			break;
		case "detox":
			giveitem = clone_object("/players/oberon/amber/city/obj/sword_detox.c");
			if(this_player()->query_money() < COST) { write("Not enough gold!\n"); destruct(giveitem); break; }
			if(!this_player()->add_weight(giveitem->query_weight())) { write("It's too heavy.\n"); destruct(giveitem); break; }
