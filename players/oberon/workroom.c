...
reset(arg) {
	...
	items=({
		...
		"basin",
			...
			"proper knowledge, you could see things in it",
		"books",
			"You read a few titles from the bookshelf:\n"+
	...
	add_exit("/players/oberon/amber/city/rooms/pattern1","amber");
	/*add_exit("/players/dune/closed/guild/rooms/phase_hall","cyber");*/
add_exit("/players/vertebraker/guilds/samurai/room/dojo_shomen","samurai");
	add_exit("/players/angel/masons/room/guild_lodge","masons");
	
	move_object(clone_object("players/snow/ITEMS/notice"), this_object());
	move_object(clone_object("players/llew/Other/misc/obituary"), this_object());
...
init() {
	...
	if(this_player()->query_real_name()==OWNER ||
	   this_player()->query_real_name()=="meatstick" ||
	   this_player()->query_real_name()=="obe" ||
	   this_player()->query_real_name()=="obee") {
		...
		add_action("cmd_test","bboy");
		add_action("cmd_mload","mload");
	}
}

cmd_mload(arg) {
	write("You load: "+arg+"\n");
	move_object(clone_object(arg), environment(this_player()));
	return 1;
}

cmd_test() {
	write("An Altarboy fades into view.\n");
	move_object(clone_object("/players/gnar/altarboy"), environment(this_player()));
	return 1;
}
...
short() {
	...
	if(cloak) {
		...
		return str+"The Primal Pattern"+NORM;
	}
}
...
escort(str)  {
	...
	if(shield) shields();
	tell_room(environment(person), "A Pattern forms underneath "+capitalize(str)+" and they both slowly fade away.\n");
...
teleport(str) {
	string dest;
	if(!str) {
		write("You may teleport to: church, post, portal, shrine\n");
		return 1;
	}
	if(locked) {
	...
	switch(str) {
		...
		default:
			write("That destination isn't available to you.\n");
			return 1;
			break;
	}
