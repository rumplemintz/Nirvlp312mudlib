...
find_time() {
#if 1  /* Dong's not working quite right yet, so leaving this enabled - Rumplemintz */
  tx = time();
  ti = tx + 21*3600;
  th = ti/3600;
   days = (th)/24;
...
#else /* Rumplemintz - clock seemed to be off, blaming DST - so i rewrote to use ctime */
    hr = atoi(ctime()[11..12]);
