...
#define JUDGE "jabroni"
#define JAMBASSADOR "salina"
#define TAMBASSADOR "slacker"
...
object SHIELD;
int recruited, ready, complete;   /* added by fred */
...
reset(arg) {
   ...
   notyet = 0;
   owner = TP;
   if(KPTS > 150){ KPTS = 150; save_ob(); return 1; }
   ...
   restore_object("players/boltar/templar/members/"+this_player()->query_real_name());
    if(!ref_flag)
 if(this_player())
  if(!this_player()->query_invis() && this_player()->query_level() < 21 && !this_player()->is_npc())
...
crestore(){
  restore_object("players/boltar/templar/members/"+environment()->query_real_name());
}
drop() {
   if (query_verb() == "drop")
...
if(this_player() && this_player()->query_real_name() != "boltar") {
...
*/
   ...
   add_action("ally_check", "ally_check");
   add_action("crim_check", "crim_check");
   ...
   add_action("cmd_recruit","recruit");
   /******End Elite Spells*******/
  ...
  /* 10/20/05 Earwax council flag not saving properly? */
...
gen_wc_bonus(){
  if( USER->query_prestige_level() ) {
    return USER->query_alignment()/165 ;
  }
  return 0;
}
...
ally_check(str){
  if(this_player()->query_guild_rank() < 9)
    return 0;
    tail("/players/boltar/templar/log/Ally");
  return 1;
}
...
tail("/players/boltar/templar/log/good_kill");
return 1; }
...
shield() {
   ...
   SHIELD = clone_object("/players/boltar/templar/shield.c");
   move_object(SHIELD, TP);
...
decorpse() {
  object blah;
   ...
   if (sscanf(file_name(corpse),"obj/corpse%s",i)!=1 &&
       sscanf(file_name(corpse),"players/dragnar/WalkingDead/obj/corpse%s",i) != 1 ) {
     return 1;
   }
...
fixtitle() {
   string title;
   if(TP->query_real_name() == JUDGE)
   {
     title = HIR+"["+HIW+"Templar"+HIR+"] "+HIW+"Judge Advocate"+NORM;
   }
   else if(TP->query_real_name() == JAMBASSADOR)
   {
     title = NORM+MAG+"["+HIW+"Templar"+NORM+MAG+"] "+HIW+"Judge Ambassador"+NORM;
   }
   else if(TP->query_real_name() == TAMBASSADOR)
   {
     title = HIB+"["+HIW+"Templar"+HIB+"] "+HIW+"Templar Ambassador"+NORM;
   }    
   else if(elite && TP->query_guild_rank() == 10)
   {
     title = RED+"["+NORM+BOLD+"Templar"+NORM+RED+"] "+NORM+BOLD+"Elite General"+NORM;
   }
   else if(elite && TP->query_guild_rank() == 9)
   {
     title = RED+"["+NORM+BOLD+"Templar"+NORM+RED+"] "+NORM+BOLD+"Elite Lieutenant"+NORM;
   }
   else if(elite)
   {
   ...
   {
     title="the "+BOLD+GRN+"["+HIW+"Templar"+HIG+"] "+HIW+"High Marshall"+NORM;
   }
...
return 1;
    ...
    if(sscanf(str, "%s %s", em, augh) != 2) {
      em = str;
    }
    em = implode(explode(em, "."), "");
    if(file_size(file="/bin/soul/_"+em+".c") > 0) {
      return (int)file->guildcast("Knights Templar", augh);
    }
}
/* $ color by verte */
str = "/players/vertebraker/closed/std/color"->filter_msg(str);
...
"Knights Templar",esc+"[0m"+esc+"[0m"+esc+"[32m"+"("+HIW+"Templar"+NORM+GRN+") "+this_player()->query_name()+esc+"[0m "+str);
/*   $ end history patch by verte $ */
myname = capitalize((string)this_player()->query_real_name());
   for(i=0; i<sizeof(list); i++) {
      cross = present("KnightTemplar",list[i]);
...
if(elite) bonus += random(4) + 1;
  /* inserted by illarion 4 dec 2004 */
  if(USER->is_testchar()) {
    tell_object(USER,"Testchar feedback: Templar weapon bonus: "+bonus+"\n");
  }
 return bonus;
}
...
int skill_check_test(object wearer, int raw_dam) {
  ...
  string weapon_type;
  shield_ob=(object)wearer->query_shield();
  weapon_ob=(object)wearer->query_weapon();
  pie=(int)wearer->query_attrib("pie");
  dex=(int)wearer->query_attrib("dex");
  align=(int)wearer->query_alignment();
  if(weapon_ob) weapon_type=(string)weapon_ob->query_type();
  /* More defensive capabilities based on weapon skill */
  if(weapon_type) {
    int wskill;
    wskill=(int)query_skill(weapon_type);
    if(wskill && align > 0 && random(dex) > 15) {
      wres=10+random(2*wskill+align/100);
      if(wres)
  ...
  if(shield && shield_ob) {
    ...
    if(align > 0 && random(pie) > 10) {
      res=10+random(2*shield+align/100);  /* 11-29% initial resistance */
      if(res)
        tell_object(wearer,"Your shield glows with a holy aura!\n");
    }
  }
  res=100*res+100*wres;
  if(wearer->is_testchar())
    tell_object(wearer,"Templar ac: "+ac_bonus+" Templar res: "+(res/100)+"\n");
  return ac_bonus+res;
}

skill_check_def(object wearer, int dam) {
 int bonus;
 object shi;
 if(USER->is_testchar()) return skill_check_test(wearer,dam);
 if(locked) return 0;
...
home(){ 
   if(TP->query_level() > CREATE-1) return 0;
...
if(AT)
        ...
        msg, msg2, msg3);
        USER->set_spell_dtype("other|good");
        return 1;
}
...
holy_ground()
{
  ...
  if(basename(environment(this_player()))[0..23] != "/players/jaraxle/templar") 
  {
    write("This spell may only be used in the Templar Castle.\n");
    return 1;
  }
  if(cost > USER->query_sp())
  {
...
if(TP->query_gender() == "female") {
 ...
 if(blah > (where2-where)){
   ...
   write("[ "+YEL+"Intox: "+HIY+((TP->query_intoxination()*100)/(TP->query_max_intox()))+NORM+"% ]  " );
   write("[ "+GRN+"Food: "+HIG+((TP->query_stuffed()*100)/(TP->query_max_stuff()))+NORM+"% ]  " );
   write("[ "+CYN+"Soak: "+HIC+((TP->query_soaked()*100)/(TP->query_max_soak()))+NORM+"% ]  " );
...
if((int)ob->query_alignment() > 0) { guild_ob->remove_kpts(12); }

   if((int)ob->query_alignment() > 0)
   if((int)t_player->query_level() < 21)
   write_file("/players/boltar/templar/log/good_kill",
  ctime()[4..15]+
  " "+(string)t_player->query_name()+
   " killed "+(string)ob->query_name()+"\n["+basename(ob)+"].\n");
if((int)ob->is_player()){
...
/* Follow additions are for the Elite Mini-quest */

query_recruited(){ return recruited; }
set_recruited(num){ recruited = num; } 

query_ready(){ return ready; }
set_ready(num){ ready = num; }

query_complete(){ return complete; }
set_complete(num){ complete = num; }
...
cmd_recruit(str)
{
  ...
  if(!str)
  {
    write("Who are you trying to recruit?\n");
    return 1;
  }
  if(!elite) return;
  if(!find_living(str))
  ...
  if(ob->query_elite())
  {
    write("They are already an elite member.\n");
    return 1;
  }
  if(ob->query_recruited())
  {
    write("They have already been recruited to become an elite member.\n");
    return 1;
  }
  ob->set_recruited(1);
  save_ob();  
  write("You recruit "+capitalize(str)+", to become an Elite Guardsman.\n");
  tell_object(find_living(str),
   "\n"+BOLD+TP->query_name()+" has recruited you to beome one of the Mighty Elite.\n"+
   "To learn of your task to become an full member of the Elite Guard seek out\n"+
   "Jacoby in the Templar Castle and ask him about the Holy Grail.  Good Luck!!\n\n"+NORM);
  write_file("/players/boltar/templar/log/recruit", ctime(time())+"  "+
     this_player()->query_name()+" recruited "+capitalize(str)+" to become an Elite Knight.\n");
  return 1;
}
