...
check_component()
{
...
#if 1
  if( !guild->query_component( component_id ) && 
      !present(component_id, tp) && !present(component_id, env) ) 
  {
  ...
  if(pouch = present("mage's component pouch", tp)) {
    if(!present(component_id, tp) && !present(component_id, env) ) {
      pouch->cmd_unpouch( component_id );
    }
  }
...
#if 0
  if( !present(component_id, tp) && !present(component_id, env) &&
      guild->query_component( component_id ) )
  {
    guild->add_component( component_id, -1 );
  }
#endif
  return 1;
}
