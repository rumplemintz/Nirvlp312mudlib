...
mixed *components_a;            /* saved components */
...
mage_autosave()
{
  ...
  if(spells && sizeof(spells))
        save_data(en->query_real_name());
  return 1;
}

save_data(name)
{
  /* Save mapping to an array for the save file */
  components_a = deconstruct_mapping( components );
...
load_data(name)
{
  ...
  is_loaded = name;
  if( err = catch( components = reconstruct_mapping( components_a ) ) ) {
    write("Error setting up components mapping..."+err+"\n");
    components = ([ ]);
    components_a = ({ });
  }
  if( !components_a || sizeof(components_a) < 1 ) {
    i = sizeof( COMPONENT_IDS );
    while( i-- ) {
      components += ([ COMPONENT_IDS[i] : 0 ]);
    }
    components_a = deconstruct_mapping( components );
  }
...
long()
{
  ...
  if( this_player() && this_player()->query_level() > 99 ) {
    int x;
    string *comps;
    comps = m_indices( components );
    write("Total size of components: "+sizeof(comps)+"\n");
    x = sizeof(comps);
    while( x-- )
      write(comps[x] + " "+components[comps[x]]+ "\n");
  }
  return;
}
...
/* components */
mapping query_components() { return components; }
int query_component( string comp_name ) {
  return components[ comp_name ];
}

add_component( string comp_name, int amt ) {
  if( !comp_name)
    return 0;
  ...
  if( member( m_indices( components ), comp_name ) >= 0 ) 
    components[ comp_name ] = components[ comp_name ] + amt;
  else
    tell_object(en, "Invalid component name being added: "+comp_name+"\n");
}
