convert_xp(int i){
	...
	if(glvl == 40 && skill_pts){ 
     write("You must spend your skill points.\n");
     return 1;
     }
	if(glvl > 59){ write("You are maxed as a Warrior.\n"); return 1; }
	if(!i){ write("convert how many experience points?\n"); return 1; }
	sscanf(i, "%d", woo);
/* Why no wizards? /boggle - Rump
...
*/
	...
	USER->query_extra_level() - 1);
        ...
        /* For wizzies again - Rump */
        if (USER->query_level() > 20) minexp = expcost;
	if(expcost > minexp) {
	...
	write_file("/players/guilds/warriors/logs/adv",
...
check_advance_2(exp){
	...
	gxp += exp;
		...
		glvl += 1;
		skill_pts += 1;
      if(random(100) < 5) skill_pts += 1;
				
		tell_object(USER,
			"\n"+HIM+"  ------ ---- "+HIW+"You Gain a Level"+HIM+" ---- ------ "+NORM+"\n"+
		"       You are now level "+glvl+".  Go back to the book to raise advanced skills .\n");
		if(glvl < 60)
                if(1000000 <= gxp) check_advance2(0);
		silent_save();
		return 1;
	}
	return 1;
...
raise_skill(str){
   if(glvl >= 40){ write("You must advance at the Books.\n"); return 1; }
	if(!str){
...
/* Added following for typos - Dec-01-2009 - Rumplemintz */
        else if (str == "first aid") { return 1; }
...
raise_or(int i){
	...
	if(random(100) < i+35){
		...
		if(random(100) < i+23){ skill_pts += 1; }
		if(random(100) < i+25){ stamina_bonus += 1; }
	}
	else return;
}
...
/* Added following for typos - Dec-01-2009 - Rumplemintz */
        else if (str == "first aid") { return 50; }
...
/* Added following for typos - Dec-01-2009 - Rumplemintz */
        else if (str == "first aid") { return 5; }
