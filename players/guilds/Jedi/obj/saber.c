  ...
  if(ignited) {
    ...
    if( ignited < 2 ) {
      ...
      tell_room(environment(USER), USER->query_name()+ " ignites "+USER->query_possessive()+
           ...
           -   -  --  --- ----  ------- SNAP HISS -------  ---- ---  --  -   -"+NORM+"\n\n\n");
      USER->RegisterWeapon(JEDI, ({ "physical", 10+SKILL, "jedi_extra_hit" }));
      set_light(1);
...
long() {
  ...
  if(ignited) {
   if( ignited < 2 )
     ...
     write("The double "+wep_color+"ignited blades"+NORM+" resonate together with a deadly hum.\n");
  }
}
