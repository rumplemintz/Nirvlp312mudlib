...
/**************************************
 Added by Gnar for Prestige Gold Perk
***************************************/
#if 1
remove_object() {
  int skill,amt;
  ...
  gold = present("moneyXX", this_object());

  if( gold )
    amt = gold->query_goldamt();

  tp = this_player();

  if( tp && prestige_gold_flag ) {
    ...
    if( skill && skill > random( skill + 4 ) ) {
      if( gold ) {
        ...
        /* If no gold, similar to BF gold gem */
        amt = corpse_level * (60 + random(10));
      }
      gold = clone_object( "/obj/money" );
      gold->set_money(amt * 15 / 10);
...
set_prestige_gold() {
  prestige_gold_flag = 1;
}
int query_prestige_gold() { return prestige_gold_flag; }
