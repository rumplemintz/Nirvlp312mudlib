...
reset(arg)
{
  ...
  if (luck2) luck = luck2;
  prestige_gold_flag = 2;
}

/***************************************************************
* Gnar - This is used to prevent cheating for the prestige
*        gold perk.
****************************************************************/
void add_money(int m) {
  prestige_gold_flag--;
  /* If add_money is called more than once, block prestige gold */
  if( prestige_gold_flag == 1 && this_player() && this_player()->is_player() )
    prestige_gold_flag = 0;

  if( prestige_gold_flag < 0 )
    prestige_gold_flag = 0;
  ::add_money(m);
}
...
query_prestige_gold() { return prestige_gold_flag; }
