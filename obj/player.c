...
/* Static Variables */
/** Gnar -- List of objs saved on the player **/
static object *treasure_obj;
...
static int top_boot_xp_msg;
static int auto_monitor;

/* Non-Static Global Variables */

int security_level; /* Rumplemintz - access privileges */
...
#define DEBUG_RUMP(x) if(find_player("rumplemintz")) \
                        tell_object(find_player("rumplemintz"), x);

/*****************************************************************
 * BEGIN PRESTIGE ADDITION 
 ****************************************************************/
int prestige_level;
int prestige_xp;
int prestige_qp;
...
mapping prestige_guild;
mixed *prestige_guild_a;
...
#define PRES_GUILD_FILE 0
#define PRES_GUILD_RANK 1
#define PRES_GUILD_EXP  2
#define PRES_GUILD_HOME 3

#define PRES_GUILD_SIZE 4

/*****************************************************************
 * END PRESTIGE ADDITION 
 ****************************************************************/
...
/*************************************************************
 * Added by Gnar 08/15/2014
 *************************************************************/
query_auto_monitor() { return auto_monitor; }
set_auto_monitor() { auto_monitor = 1; }
clear_auto_monitor() { auto_monitor = 0; }
...
/* Changing this a bit per a player suggestion to count invis pk'ers as
  ...
  treasure_str=allocate(NUMOBS_STOR);
...
shoutin () {
  ...
  if(lvl > 0) {
    chan_msg(nom + " ( player " + lvl + 
      (ex_lv ? ("+" + ex_lv) : "") +
      (plev ? HIG+"{"+plev+"}"+NORM : "") + " ) has entered the game.\n", "msg");
    return 1;
  }
...
shoutout() {
  ...
  if(lvl > 0) {
    chan_msg(nom + " ( player " + lvl + 
      (ex_lv ? ("+" + ex_lv) : "") + 
      (plev ? HIG+"{"+plev+"}"+NORM : "") + " ) has left the game.\n", "msg");
    return 1; 
  }
  else if(wi)
...
*/
  ...
  if( err = catch( prestige_guild = reconstruct_mapping( prestige_guild_a ) ) ) {
    write("Error setting up mapping..."+err+"\n");
    prestige_guild = ([ ]);
    prestige_guild_a = ({ });
  }
#endif
...
query_treasure() { return treasure;}

object *query_treasure_obj() { 
  if( !treasure_obj ) treasure_obj = ({ });
  return treasure_obj;
}
...
reset(arg) {
  if (!arg) {
    ...
    if(!atrbr)
      atrbr = ({charisma,dexterity,intelligence,luck,magic_aptitude,piety,
                stamina,stealth,strength,will_power,});
    if(!AnsiPref)
      AnsiPref = ({ "page", 0, "tell", 0, "gossip", 0, "risque", 0, "junk", 0,
                    "babble", 0, "newbie", 0, "msg", 0, "announce", 0 });
    ...
    if( !prestige_skills )
      prestige_skills = allocate( sizeof( VALID_PRES_SKILL ) );

    prestige_guild = ([ ]);
      
    /** Resize arrays in case more are added later **/
    if( sizeof( prestige_skills ) < sizeof( VALID_PRES_SKILL ) ) 
      prestige_skills += allocate( sizeof( VALID_PRES_SKILL ) -
                         sizeof( prestige_skills ) );
...
short() {
  ...
  if (this_player() &&
      ...
      (ob = present("sight_short_modifier", this_player())))
...
long() {
  ...
  if(racetmp[0] == 'a' || racetmp[0] == 'e' || racetmp[0] == 'i' ||
    ...
    a_or_an = 1;
  racetmp = tmptmp;
  write((this_player()==this_object() ? "You are" : cap_name + " is") +
        (a_or_an ? " an" : " a") + " " + racetmp + ", " +
         phys_at[1] + " feet " + phys_at[2] + " inches tall, " +
         phys_at[3]+" lbs.\n");
  if(pregnancy) 
    preggers_long(me_me, phys_at[3], (age-pregnancy));
  if (player_killing) 
    write(RED + (me_me ? "You" : cap_name) +
...
status player_id_check(string arg) {
#ifndef __LDMUD__ /* Rumplemintz */
  return (member_array(arg, player_ids) > -1);
#else
  return (member(player_ids, arg) > -1);
#endif
}
...
query_pretitle() {
  ...
  return ((tlcol=get_ansi_pref("pretitle")) != 0 ? tlcol+pretitle+NORM
                                                 : pretitle);
}
...
set_wiz_level(string key){
  ...
  if(object_name(previous_object()) != "bin/wiz/_promote") {
  ...
  tell_object(myself, "You were " + verb + " to level " + key + " by " +
              ...
              ".\n");
  tell_object(myself, "Remember to read the help files to determine your " +
                      "new abilities.\n");
  log_file("PROMOTIONS", "[" + ctime()[4..15] + " " + ctime()[20..23] + "] " +
           capitalize(name) + " was " + verb + " to Level " + key + " by " +
...
do_quit(str) {
  if(str) return 0;
  if (file_size("/players/" + this_player()->query_real_name() + "/logout") < 1)
    return quit();
...
do_quit2(str) {
  if (lower_case(str) == "y")
    RUN_COMMAND_FILE->run_command_file("/players/" +
...
valid_attack(ob) {
  ...
  /* Nosferatu cannot pk within their own guild */
  if (ob->query_guild_name() == "Nosferatu" && guild_name=="Nosferatu")
    return 0;
/*
  if (ob->query_guild_name() == "Elementalist") {
    write("That player is beta testing a PK guild.  Please wait until\n"+
          "beta testing is over.\n");
    return 0;
  }
  if (this_player()->query_guild_name() == "Elementalist" && ob->is_player()) {
...
*/
  ...
  if(fight_area && ob->query_fight_area() && fight_area == object_name(here)) {
  ...
  if(ob->query_pl_k() > 1 || this_object()->query_pl_k() > 1) {
    ...
    write("One or both pk flags are set to spar.  " +
          "This is not a sparing area.\n");
    return 0;
  } 
...
static autoheal() {
  ...
  old_sp = spell_points;
  /*****************************************************************
   * BEGIN PRESTIGE HP REGEN ADDITION 
   ****************************************************************/
  if( PRES_SKILL_REGHP > random( PRES_SKILL_REGHP + 4 ) ) {
    hit_point += intoxicated ? 22 : 3; 
    if (hit_point > max_hp)
      hit_point = max_hp;
  }
  /*****************************************************************
   * END PRESTIGE HP REGEN ADDITION 
   ****************************************************************/
  /*****************************************************************
   * BEGIN PRESTIGE SP REGEN ADDITION 
   ****************************************************************/
  if( PRES_SKILL_REGSP > random( PRES_SKILL_REGSP + 4 ) ) {
    spell_points += intoxicated ? 22 : 3; 
    if(spell_points > max_spell)
      spell_points=max_spell;
  }
  /*****************************************************************
   * END PRESTIGE SP REGEN ADDITION 
   ****************************************************************/
...
heart_beat() {
  ...
  if (is_interactive) {
    ...
    if (infuse > 0)
      infuse -= 1+random(3);
    if (attacker_ob) {
      if (stuffed > 0 && !random(3))
        stuffed--;
      if (soaked  > 0 && !random(3))
        soaked--;
    }
    else {
      if (stuffed > 0)
    ...
    /*****************************************************************
     * BEGIN PRESTIGE DETOX ADDITION 
     ****************************************************************/
    if( PRES_SKILL_DETOX > random( PRES_SKILL_DETOX + 4 ) ) {
      int det_amt;

      det_amt = 3 + PRES_SKILL_DETOX / 20;
...
#include "/obj/player/prego_hb.c"
  ...
  if (attacker_ob) {
    ...
    if(attacker_ob) {
      ...
      if (mon == 1 || mon == 2) print_monitor();
      if (mon == 3 ) set_auto_monitor();
    }
  }
  else 
    dam_taken_this_round = 0;
  ...
  if(!noidlequit) {
    if (!idlewarn && is_interactive && myidletime > idletime) {
      write("You have been idle for " + idletime/60 +
            " minutes. You will be logged off in 5 minutes.\n");
      idlewarn = 1;
    }
    if(idlewarn && is_interactive && myidletime < idletime) 
      idlewarn = 0;
...
static move_player_to_start(where) { 
  ...
  if (other_copy && other_copy != this_object()) {
    if (interactive(other_copy)) {
      ...
      if (!exec_flag) {
        ...
        other_copy->set_saved_where(object_name(environment(other_copy)));
  ...
  if(level >= CREATE) {
    ...
    if(where != "room/adv_guild") cat ("/MANAGEMENT");
...
#if 0 /* Validate the need for this still - Rumplemintz */
#include "/obj/player/dr/closed_guild_check.c"
#endif

  setup_atrbr();
  if (treasure_str[0]) {
    write("Restoring inventory items...\n");
    treas_c = 0;
...
add_hit_point(arg) {
  ...
  if (thypl) {
    if (level < EXPLORE &&
        thypl->query_interactive() &&
        thypl != this_object()) {
      if(thypl->query_level() > EXPLORE)
        log_file("POINTS", name + " hp-add " + arg + thypl->query_real_name() +
                 " " + ctime() + "\n");
      else
        ...
        if (thypl->query_guild_name() == "healer") { 
          ...
          INF_MIN = infuse+3*arg/2;
          INF_MAX = (INFUSE_LIMIT)/2;
          INF_ADD = 3*arg/2;
        }
        if (((INFUSE_LIMIT) < INF_MIN ||infuse >INF_MAX) && arg > 0) {
          int blah;
          blah = 2*((INFUSE_LIMIT) - infuse)/3;
          tell_object(thypl, cap_name + " must wait before absorbing " + arg +
                             " hit points.\n");
          if (infuse < INF_MAX && blah > 0 && arg > 0)
            tell_object(thypl, cap_name + " may absorb up to " + blah +
                               " hit points.\n");
          tell_object(this_object(),
                      "You must wait before absorbing more hit points.\n");
          arg = 0;
          return 1;
        }
        if(arg > 0) infuse += INF_ADD;
      }
    } /* end the thypl check here so call_outs can use this */
...
add_spell_point(arg) {
  ...
  if(thypl) {
    if (level < EXPLORE &&
        ...
        thypl != this_object()) {
      if( thypl->query_level() > EXPLORE) 
        log_file("POINTS", name + " sp-add " + arg + thypl->query_real_name() +
                           " " + ctime() + "\n");
      else {
        if(environment(thypl) != environment(this_object())) {
          arg = 0;
          tell_object(thypl, cap_name + " is not here.\n");
          tell_object(this_object(),
                      "Attempted Spell Point transfer Aborted.\n");
          return 1;
        }
        if (thypl->query_guild_name() == "healer") { 
        ...
        if(((INFUSE_LIMIT) < INF_MIN ||infuse >INF_MAX) && arg > 0) {
          ...
          if(infuse < INF_MAX && blah > 0 && arg > 0)
             tell_object(thypl, cap_name + " may absorb up to " + blah +
                                " spell points.\n");
          tell_object(this_object(),
                      "You must wait before absorbing more spell points.\n");
          arg = 0;
          return 1;
        }
        if(arg > 0) infuse += INF_ADD;
  ...
  spell_points += arg;
  if (arg < 0 && guild_name == "Knights Templar" &&
                 this_object()->query_mounted())
    spell_points += 2*arg/3;
  if (spell_points > max_spell) spell_points = max_spell;
  if( old_sp != spell_points )
...
drop_all(int verbose) {
  ...
  if (!myself || !living(myself))
    return;
  if( verbose > 1 ) {
    /** Move stored objs to the player **/
    inv = deep_inventory(myself);
    for( i=sizeof(inv); i--; ) {
      if( environment( inv[i] ) != myself && member( treasure_obj, inv[i] ) >= 0 ) {
        move_object( inv[i], myself );
      }
    }
  }
  ...
  while(ob) {
    string out;
    next_ob = next_inventory(ob);
    /** Only drop items not saved **/
    if( verbose < 2 || member( treasure_obj, ob ) < 0 ) {
      ...
      tell_object( myself, ob->short() + " SAVED.\n");
    ob = next_ob;
  }
}
...
add_exp(x) {
  ...
  if( previous_object() ) {
    sscanf(object_name(previous_object()),"%s#%s",tmpo,tmpi);
    if(tmpo != "obj/party" &&
       object_name(previous_object()) != "obj/partymaster")
      party_flag = 0;
  }
  if (this_player() && previous_object() && this_player() != this_object() &&
      this_player()->query_level() > 1 && query_ip_number(this_player()) &&
      level < 20 ) {
    if( !party_flag ) {
      log_file("EXPERIENCE", ctime() +" "+ name +"("+ level + ") " + e +
                             " exp by " + this_player()->query_real_name() + 
                             "(" + this_player()->query_level() + ")" +"\n");
      log_file("EXPERIENCE", object_name(previous_object())+"-^\n");
    }
  }
  if (is_interactive) {
    int curr_boot_xp;
    ...
    curr_boot_xp = "obj/exp_store.c"->query_player_exp( cap_name );
    if( curr_boot_xp > top_boot_xp ) {
      set_top_boot_xp( curr_boot_xp );
    }
    if( lr_period_start && e > 0 ) {
      lr_exp += e;
      if( lr_exp > lr_top_exp )
        lr_top_exp = lr_exp;
      if( !lr_flag && lr_exp > lr_required_exp ) {
        give_login_reward();
      }
    }
    /**************************************************************
     *  Guild shadow objects/party objects need to do this if they
     *  want to store top kill, as previous_object will not be a
     *  an NPC in that case. 
     **************************************************************/
    if( !party_flag && previous_object() && previous_object()->is_npc() ) 
      set_top_kill( e, previous_object() );
...
remove_ghost_finish() {
  ...
  if (!ghost)
    return 0;
  if(PKGhostCounter)
    return (tell_object(this_object(),
                        "\nYou may not be revived from death for another " +
                        PKGhostCounter + " seconds.\n\n"), 1);
...
void set_hunted(object x) { hunted = x; }
...
unset_invs_sp() {
  is_invis = invs_flag = invs_count = 0;
  return 1;
}
...
status query_no_give() { return no_give; }
...
#else
    save_object("pfiles/" + name[0..0] + "/" + name);
#endif
...
illegal_patch(what) {
  ...
  log_file("ILLEGAL", capitalize(this_player()->query_real_name()) + " " +
                      ctime() + " " + what + "\n");
  return 0;
}
...
compute_treasure() {
    ...
    /** Gnar 02/06/2015
      * Loop through all inventory making 2 arrays 
      * 1. Objects that are kept
      * 2. Objects that are not kept **/
    for( x=0; x<s && sizeof(kept) < NUMOBS_STOR; x++) {
      ob = obs[x];
      ...
      if( ob->short() ) {
        if(!ob->query_save_flag() && !ob->query_auto_load() &&
           !ob->id("GI") &&
           ((ob->generic_object() && ob->locker_arg()) || !ob->generic_object()) )
        {
          if( ob->queryKept() )
            kept += ({ ob });
          else
    ...
    /** Recreate the obs array, putting kept objects first so they are saved first **/
    obs = kept;
    obs += nonkept;

    s=sizeof(obs);
    b=0;    
    for(x=0; x<s && b < NUMOBS_STOR; x++) {
       ob = obs[x];
       if(!ob) continue;
       str = object_name(ob);
...
void get_new_player_object() {
  ...
  if (!(fname == "obj/wizard" || fname == "obj/player")) return;
  other_copy = this_object();
#ifdef LOG_RENEWALS
  log(LOG_RENEWALS, fname, ((security_level) ? "obj/wizard" : "obj/player"));
#endif /* LOG_RENEWALS */
  if (security_level)
#if 0
    clone_object("obj/wizard");
#else
...
/* The optional argument of 'object c' allows to call valid_write with a
   different object (other than this_object()) - not sure we want that
   -- Rumplemintz
 */
nomask mixed valid_write(string str, object c) {
  /* All of these temp variables are ridiculous... Rump */
  string who, tmpa, tmpb, tmpc, file, temp;
...
#ifdef __LDMUD__
  write("Valid Write: "+str+" ob: "+file_name(c)+"\n" );
#else
#if 0
  if( query_real_name() == "dreadbane" )
    write("Valid Write: "+str+" ob: "+file_name(c)+"\n" );
#endif
#endif
...
#if 0
  write("Valid Write: str is " + str + "\n" +
        "Calling make_path(str)\n");
#endif

  str = make_path(str);

#if 0
  write("Valid Write: str is now " + str + "\n");
#endif
  ...
  if (level >= CREATE &&
      str == "/log/WR/WORK_REPORT" &&
      previous_object() && object_name(previous_object()) == "bin/wiz/_wlog") {
...
#ifdef __LDMUD__
  fname = (c ? object_name(c) : 0);
#endif

  if (caller() != myself || !c)
    c = caller();

/* If the caller is not this_object, then get the object_name of the caller,
  ...
  if (extract(object_name(c),0,3)=="room" ||
      extract(object_name(c),0,2)=="obj" ||
      extract(object_name(c),0,2)=="bin" ||
      extract(object_name(c),0,5)=="secure" ||
      extract(object_name(c),0,2)=="lib" ||
      extract(object_name(c),0,2)=="sys" ||
      extract(object_name(c),0,6)=="daemons" ||
      extract(object_name(c),0,5)=="kernel" ||
      extract(object_name(c),0,5)=="global" ||
      extract(object_name(c),0,4)=="basic" ||
      extract(object_name(c),0,6)=="complex" ||
      extract(object_name(c),0,5)=="closed")
    c=this_object();
...
/* Disallow any of these combinations */
  ...
  if (sscanf(str, "/players/%s/%s", who, file) == 2) {
    ...
    if(level < ELDER && level > 20)
      if(sscanf(str,"%s" + name + "%s",tpa, tpb) != 2)
        if(sscanf(object_name(c), "players/" + who + "/%s", temp) != 1) {
          string vvverb;
          vvverb = query_verb();
    ...
    if (sscanf(object_name(c), "players/" + who + "/%s", temp)==1) {
#if 0
      if( query_ip_number( this_object() ) == "66.67.15.93" ) {
        tell_object( this_object(), "\t\t/players c:"+object_name(c)+".\n");
      }
#endif
      return "players/" + who + "/" + file;
    }
  ...
  if (sscanf(str, "/players/%s.o", file) == 1)
    if(level >= ELDER ||
       (prev_caller && object_name(prev_caller) == "obj/simul_efun"))
  ...
  if(sscanf(str, "/pa/log/%s", who) == 1) {
    if((prev_caller && (basename(prev_caller) == "/obj/soul" ||
                        basename(prev_caller) == "/closed/wiz_soul")) ||
        level >= SENIOR) {
      return "pa/log/"+who;
    }
  }
  ...
  if((sscanf(str, "/room/housing/%s",file) == 1) && prev_caller &&
      (object_name(prev_caller)[0..11]=="room/housing"))
  ...
  if ((sscanf(str, "/room/%s", file) == 1) && level >= EDIT_STANDARD)
    ...
    if(sscanf(str,"/pfiles/%s",file))
      if(level >= ELDER || (prev_caller &&
         object_name(prev_caller) == "obj/simul_efun"))
         return "pfiles/" + file;
...
nomask string valid_read(string arg) {
...
#if 0
  tell_object( this_object(), "valid_read: "+arg+".\n");
#endif
  ...
  if (c != this_object() && !in_editor(this_object()))
    if (extract(object_name(c),0,3)=="room" ||
        extract(object_name(c),0,2)=="obj" ||
        extract(object_name(c),0,2)=="bin" ||
        extract(object_name(c),0,7)=="post_dir" ||
        extract(object_name(c),0,5)=="closed")
      c = this_object();
  if (level < ELDER)
...
/* The /pa/ stuff is borked on LD - Rump */
...
set_fight_area() {
  fight_area=object_name(environment(this_object()));
  return 1;
}
clear_fight_area() { fight_area=0; return 1; }

check_fight_area() {
  if(fight_area && fight_area == object_name(environment(this_object())))
    return 1;
  fight_area = 0;
  return 0;
}
...
setup_broadcast(chanstrg) {
  ...
  /** Only charge sps if it is a game channel that is not free -- Gnar **/
  if( member(FREE_CHANNELS, chanstrg) < 0 && member(ALL_CHANNELS, chanstrg) >= 0 && level > 5) {
    if (spell_points < 0) {
...
/* change height and weight  1=height feet, 2=height inches, 3=weight */
    ...
    if(num==4 && arg == -9)
      log_file("b_amulet","-9 added "+object_name(caller())+"\n");
    phys_at[num] += arg;
    return 1;
}
...
set_max_hp() {
  ...
  /*****************************************************************
   * BEGIN PRESTIGE HP BONUS ADDITION 
   ****************************************************************/
  if( PRES_SKILL_HPS > 0 ) {
    max_hp += level * 6 / 5 + PRES_SKILL_HPS * 2;
  }
  /*****************************************************************
   * END PRESTIGE HP BONUS ADDITION 
   ****************************************************************/    
...
set_max_spell() {
  ...
  /*****************************************************************
   * BEGIN PRESTIGE SP BONUS ADDITION 
   ****************************************************************/
  if( PRES_SKILL_SPS > 0 ) {
    max_spell += level * 6 / 5 + PRES_SKILL_SPS * 2;
  }
  /*****************************************************************
   * END PRESTIGE SP BONUS ADDITION 
   ****************************************************************/  
...
move_player(dir_dest) {
  ...
  && !random(10*hit_point/max_hp)) {
    call_other("/obj/play/move_tracker","add_blood",object_name(environment()));
    if(is_testchar())
      write("Blood trail added.\n");
  }
  ::move_player(dir_dest);
  if(player_killing && environment()
  && call_other("/obj/play/move_tracker","check_blood",object_name(environment())))
...
query_extra_exp_check() {
  ...
  return exp_active_flag || query_is_bday() || present("double_xp_object", this_object() );
}
...
set_prestige_level( int amt ) { 
  if( !testchar_var ) 
    log_file("PRESTIGE_ADVANCE", ctime() + " " +query_name()+" adv from "+prestige_level+" to "+amt+" with XP: "+query_exp()+"\n");
  prestige_level = amt;
  save_me();
}
...
query_prestige_score() { 
  return (experience / 1000) + prestige_xp + 
         (prestige_level * 75000 ) +
         ( (prestige_qp + quest_point) * 100); 
}
...
query_prestige_skill( string skill ) {
  ...
  idx = member( VALID_PRES_SKILL, skill );
  
  if( idx < 0 )
    return 0;
    
  return prestige_skills[idx];
}
...
string *
query_prestige_guilds() {
  return m_indices( prestige_guild );
}

mixed *
query_prestige_guild( string gname ) {
  int idx;

  if( !prestige_guild[ gname ] )
    return 0;

  return prestige_guild[ gname ];
}

set_prestige_guild() {
  int idx;

  if( !guild_name || !guild_file )
    return 0;

  /* add entry */
  if( !prestige_guild[ guild_name ] ) {
    prestige_guild += ([ guild_name : ({ guild_file,
                                    guild_rank,
                                    guild_exp,
                                    home, })
                      ]);
...
#ifndef __LDMUD__
  prestige_guild_a = deconstruct_mapping( prestige_guild );
#endif
  save_me();
}

add_prestige_guild( string gname, mixed *guild_info ) {

  if( !gname || !guild_info || sizeof(guild_info) != sizeof(VALID_PRES_GUILD) ) {
    return 0;
  }

  if( prestige_guild[ gname ] ) {
    prestige_guild[ gname ] = guild_info;
  }
  else {
    prestige_guild += ([ gname: guild_info ]);
  }
...
mixed
query_prestige_guild_data( string gname, string attr ) {
  int idx;

  if( !attr || !strlen(attr) || !prestige_guild[ gname ] )
    return 0;

  attr = lower_case( attr );

  idx = member( VALID_PRES_GUILD, attr );

  if( idx < 0 )
    return 0;

  return prestige_guild[ gname ][idx];
}

restore_prestige_guild( string gname ) {
  string err;
  object gob;

  if( !prestige_guild[ gname ] ) {
    write("Error restoring "+gname+" prestige guild...\n");
    log_file("PRESTIGE_ERROR", ctime() + " " +query_name()+" failed to restore "+gname+" guild.\n");
    return 1;
  }

  /** Restore guild status **/
  guild_name = gname;
  guild_file = prestige_guild[ gname ][PRES_GUILD_FILE];
  guild_rank = prestige_guild[ gname ][PRES_GUILD_RANK];
  guild_exp = prestige_guild[ gname ][PRES_GUILD_EXP];
  home = prestige_guild[ gname ][PRES_GUILD_HOME];

  /** Clone guild object **/
  if( err = catch( gob = clone_object( guild_file ) ) ) {
    write("Error restoring "+gname+" prestige guild...\n");
    log_file("PRESTIGE_ERROR", ctime() + " " +query_name()+" failed to clone "+guild_file+" guild file.\n");
    return 1;
  }
...
string *valid_prestige_skill() { return VALID_PRES_SKILL + PRES_SKILL_DESC; }
...
mixed *query_top_kill() { return top_kill; }

void set_top_kill( int exp, object corpse ) {
  string mname;
  object rm;
  
  if( corpse ) {
    rm = environment( corpse );
    mname = (string) corpse->query_name();
  }
    
  "/obj/exp_store.c"->kill_collector( cap_name, exp, mname, time() );

  if( !top_kill || top_kill[0] < exp ) {
    tell_object( this_object(), HIY+"You just beat your top kill!\n"+NORM);
    if( !is_testchar() && query_level() < EXPLORE && (!top_kill || exp - top_kill[0] > 1000) )
      chan_msg( NORM+"\n(junk) " +HIY+cap_name +NORM+ " just crushed "+this_object()->query_possessive()+" top kill record!\n", "junk" );
    top_kill = ({ exp, corpse ? (string) corpse->query_name() : "Unknown", rm ? (string) rm->short() : "Unknown", time() });
  }
}

int query_top_boot_xp() { return top_boot_xp; }

void set_top_boot_xp( int top_xp ) {
  if( !top_boot_xp_msg ) { 
    tell_object( this_object(), HIY+"You just beat your top boot experience!\n"+NORM);
    if( !is_testchar() && query_level() < EXPLORE )
      chan_msg( NORM+"\n(junk) " +HIY+cap_name +NORM+ " just beat "+this_object()->query_possessive()+" top boot exp record!\n", "junk" );
  }
  top_boot_xp_msg = 1;
  top_boot_xp = top_xp;
}
...
void transfer_extra_eq() {
  ...
  if (i_flag)
    tell_object(this_object(),
                "Due to a bug, you were overloaded and your excess equipment" +
...
status secure(int sec_level) {
  if (this_player() && !query_ip_number(this_player())) return 0;
...
nomask int query_security() {
  ...
  if (!previous_object()) return security_level;
  if (!(sscanf(file_name(previous_object()), "obj/player" + "#%d", i) ||
        sscanf(file_name(previous_object()), "obj/wizard" + "#%d", i)))
    return 0;
...
#endif
#ifdef LOG_SPONSOR
  log(LOG_SPONSOR, "Lvl: " + level + ", Sec Lvl: " + old_level,
